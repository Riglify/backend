...
app.get("/test-rbxm-avatar/:userID", async (req, res) => {
    ...
    try {
        ...
        console.log(`🟥 FULL AVATAR RBXM TEST: ${userID}`);

        const script = `
local Players = game:GetService("Players")
local SerializationService = game:GetService("SerializationService")

print("🟥 Creating avatar for user ${Number(userID)}...")

local success, avatarModel = pcall(function()
    return Players:CreateHumanoidModelFromUserIdAsync(${Number(userID)})
end)

if not success then
    warn("❌ Failed to create avatar:")
    warn(avatarModel)

    return {
        ReturnValues = {
            "FAILED_AVATAR_CREATION"
        }
    }
end
...
print("✅ Avatar model created!")
print("📦 Serializing avatar...")
...
if not serializeSuccess then
    warn("❌ Avatar serialization failed:")
    warn(result)
...
`;
        ...
        if (task.state !== "COMPLETE") {
            throw new Error(
                task.error?.message ||
                `Roblox task failed with state ${task.state}`
            );
        }

        if (!task.binaryOutputUri) {
            throw new Error(
                "Roblox returned no binary output."
            );
        }
        ...
        const rbxm = Buffer.from(binaryResponse.data);

        if (!rbxm.length) {
            throw new Error("Roblox returned an empty RBXM.");
        }

        console.log(
            `🟢 FULL AVATAR RBXM CREATED: ${rbxm.length} bytes`
        );
...
if (mtlText) {
    archive.append(mtlText, {
        name: `${username}.mtl`
    });
}

for (const texture of textureFiles) {
    archive.append(texture.buffer, {
        name: `textures/${texture.name}`
    });
}
...
return;
  ...
  if (assetId === "all_rbxm") {
    console.log(`🟥 Starting FULL AVATAR RBXM export for user ${targetUserId}`);
...
local success, avatarModel = pcall(function()
    return Players:CreateHumanoidModelFromUserIdAsync(${Number(targetUserId)})
end)
...
print("🟢 Riglify FULL AVATAR RBXM EXPORT COMPLETE!")
...
`;
    ...
    if (task.state !== "COMPLETE") {
        throw new Error(
            task.error?.message ||
            `Roblox RBXM export failed with state ${task.state}`
        );
    }

    if (!task.binaryOutputUri) {
        throw new Error(
            "Roblox completed the RBXM export but returned no binary output."
        );
    }
    ...
    if (!rbxm.length) {
        throw new Error(
            "Roblox returned an empty RBXM."
        );
    }

    console.log(
        `🟢 RBXM export complete: ${rbxm.length} bytes`
    );
    ...
    res.setHeader(
        "Content-Disposition",
        `attachment; filename="Riglify_${targetUserId}.rbxm"`
    );
